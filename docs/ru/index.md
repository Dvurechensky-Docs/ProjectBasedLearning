---
layout: default
lang: ru
title: 'Обучение на основе проектов'
seo_title: 'Обучение программированию на проектах: Python, JavaScript, Rust, C#'
description: 'Подборка практических проектов, руководств и идей для изучения Python, JavaScript, Rust, C#, веб-разработки, backend, мобильной разработки и портфолио программиста.'
h1: 'Учись программированию через реальные проекты'
date: 2025-08-26
published: true
---

## Проекты и руководства по языкам программирования

Этот портал помогает искать практические задания и туториалы: веб-приложения, API, игры, парсеры, операционные системы, мобильные приложения, data-проекты и упражнения для портфолио.

Страницы хорошо попадают в запросы вроде: проекты для начинающих программистов, идеи проектов на Python, проекты JavaScript для портфолио, Rust tutorials, C# backend проекты, HTML CSS практика и web development projects with source code.

Портал связан с другими материалами Dvurechensky. Для API-практики и backend-разработки пригодится [API-список](https://dvurechensky-docs.github.io/Internet/). Для чистой структуры репозиториев используй [Gitignore](https://dvurechensky-docs.github.io/Gitignore/). Для frontend-проектов хорошо дополняет эту подборку [Web Frontend Knowledge](https://dvurechensky-docs.github.io/WebFrontendKnowledge/), а для .NET-трека можно продолжить в [Basic Knowledge C#](https://dvurechensky-docs.github.io/BasicKnowledgeCSharp/), [Patterns C# Programming](https://dvurechensky-docs.github.io/PatternsCSharpProgramming/) и [Interview C# .NET](https://dvurechensky-docs.github.io/InterviewCSharpNet/).

Официальная страница автора: [Dvurechensky](https://dvurechensky.pro). Из связанных проектов также полезны [Lizerium Portal](https://lizup.ru/) и русский сервис [Shields.io](https://shields.dvurechensky.pro/).

## Быстрый старт

- [Поиск по порталу](search.md)
- [Проекты на Python](categories/Python.md)
- [Проекты на JavaScript](categories/JavaScript.md)
- [Проекты на Rust](categories/Rust.md)
- [Проекты на C#](categories/C#.md)
- [Проекты HTML и CSS](categories/HTML_CSS.md)

## Все категории

- [C/C++](categories/C++.md)
- [C#](categories/C#.md)
- [Clojure](categories/Clojure.md)
- [Dart и Flutter](categories/Dart.md)
- [Elixir](categories/Elixir.md)
- [Erlang](categories/Erlang.md)
- [F#](categories/F#.md)
- [Go](categories/Go.md)
- [Haskell](categories/Haskell.md)
- [HTML и CSS](categories/HTML_CSS.md)
- [Java](categories/Java.md)
- [JavaScript](categories/JavaScript.md)
- [Kotlin](categories/Kotlin.md)
- [Lua и LÖVE](categories/Lua.md)
- [OCaml](categories/OCaml.md)
- [PHP](categories/PHP.md)
- [Python](categories/Python.md)
- [R](categories/R.md)
- [Ruby](categories/Ruby.md)
- [Rust](categories/Rust.md)
- [Scala](categories/Scala.md)
- [Swift](categories/Swift.md)
- [Дополнительные ресурсы](categories/additionals.md)
